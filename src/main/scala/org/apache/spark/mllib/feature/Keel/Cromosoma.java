 ...
 * @since JDK1.5
 */
public class Cromosoma implements Comparable, Serializable {
    ...
    private double fitnessAc;
    private boolean evaluado;
    private boolean valido;
    private MersenneTwister generador = new MersenneTwister();
     ...
     */
    public Cromosoma(int K, int size, double dMatrix[][], double datos[][], boolean distanceEu) {
        ...
        for (i = 0; i < size; i++) {
            for (j = 0; j < K; j++) {
                vecinos[i][j] = obtenerCercano(vecinos[i], j, dMatrix, i, datos, distanceEu);
            }
        }
    }
     ...
     * @param distanceEu True= Euclidean distance; False= HVDM
     */
    public void mutation(int K, double pMut, double dMatrix[][], double datos[][], boolean distanceEu) {
     ...
     */
    public int obtenerCercano(int vecinos[], int J, double dMatrix[][], int index, double datos[][], boolean distanceEu) {
        ...
        if (dMatrix == null) {
            ...
            if (minPos == cuerpo.length)
                return 0;
            minDist = distancia(datos[index], datos[minPos], distanceEu);
            for (i = minPos + 1; i < cuerpo.length; i++) {
                if (cuerpo[i]) {
                    ...
                    if (cont) {
                        dist = distancia(datos[index], datos[i], distanceEu);
                        if (minDist > dist) {
                            minPos = i;
                            minDist = dist;
                        }
    ...
    public void evaluacionCompleta(int nClases, int K, int clases[]) {
        ...
        //System.out.println("fitness: " + fitness + " fitnessAc: " + fitnessAc);
    }
     ...
     */
    public double optimizacionLocal(int nClases, int K, int clases[], double dMatrix[][], double umbral, double datos[][], boolean distanceEu) {
        ...
        for (j = 0; j < visitas.length; j++) {
            pos = Randint(j, visitas.length - 1);
            tmp = visitas[j];
        ...
        i = 0;
        while (i < n) {
            ev = evaluacionParcial(nClases, K, clases, visitas[i], dMatrix, umbral, datos, distanceEu);
            if (ev >= 0) {
                n--;
                i = 0;
                visitas = new int[n];
     ...
     */
    public double evaluacionParcial(int nClases, int K, int clases[], int ref, double dMatrix[][], double umbral, double datos[][], boolean distanceEu) {
        ...
        for (i = 0; i < cuerpo.length; i++) {
            ...
            for (j = 0; j < K; j++) {
                if (vecinos[i][j] == ref) {
                    evaluar = true;
                    vecinosTemp[i][j] = obtenerCercano(vecinosTemp[i], j, dMatrix, i, datos, distanceEu);
                } else {
